export default class adminRecipesItemDetailCtrl {
    constructor(
        RecipesItemService,
        $stateParams,
        $translate,
    ...
    changeRecipeInfo() {
        ...
        const _onSuccess = (res) => {
            // this.supplier = res.data.data;
            this.notify('admin.recipes.recipe.recipe_updated', 'success', 3000);
        ...
        const _onError = (err) => {
            ...
            if (err.data.errorCode === 29) {
                this.notify('admin.recipes.recipe.recipe_email_exists', 'danger', 8000);
            } else if (err.data.errorCode === 30) {
                this.notify(
                    'admin.recipes.recipe.recipe_mobilePhone_exists',
                    'danger',
                    8000
                );
            }
    ...
    customOp(item) {
        return (
            this.$translate.instant('admin.recipes.payment.every') +
            this.$translate.instant(`admin.recipes.payment.${item.interval}`, {
...
adminRecipesItemDetailCtrl.$inject = [
    'RecipeService',
    'TransactionsService',
    '$stateParams',
    '$translate',

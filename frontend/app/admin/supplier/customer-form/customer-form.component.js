...
class CustomerFormCtrl {
    constructor(CustomerService, $translate, $rootScope, Upload,
    AppConstants) {
    ...
    $onInit() {
        ...
        // $('#customerFormModal').on('hide.bs.modal', () => {
        //     ctrl.resetForm();
        // });
    ...
    createCustomer(customerForm) {
       ...
       // this.formData.commercialRegisterExpireDate = moment(this.formData.commercialRegisterExpireDate).format('YYYY-MM-DD');
        this._CustomerService.createCustomer(this.formData, true)
            .then(
                (res) => {
                    this.isSuccess = true;
                    this.message = 'admin.customers.create-customer.message.success_creation';
                    this.notify(this.message, 'success', 3000);
                    this.$rootScope.$broadcast('getCustomers');
                ...
                (err) => {
                    ...
                    if (err.data) {
                        if (err.data.errorCode === 20) {
                            this.isFailure = true;
                            this.message = 'admin.customers.create-customer.message.expireDate_exceed';
                            this.notify(this.message, 'danger', 7000);
                        } else if (err.data.errorCode === 21) {
                            this.isFailure = true;
                            this.message = 'admin.customers.create-customer.message.expireDate_todayOrLess';
                            this.notify(this.message, 'danger', 7000);
                        } else if (err.data.errorCode === 22) {
                            this.isFailure = true;
                            this.message = 'admin.customers.create-customer.message.expireDateHijri_exceed';
                            this.notify(this.message, 'danger', 7000);
                        } else if (err.data.errorCode === 23) {
                            this.isFailure = true;
                            this.message = 'admin.customers.create-customer.message.expireDateHijri_todayOrLess';
                            this.notify(this.message, 'danger', 7000);
                        } else if (err.data.errorCode === 3) {
                        // alert('already have account');
                            this.isFailure = true;
                            this.message = 'admin.customers.create-customer.message.already_have_account';
                            this.notify(this.message, 'danger', 5000);
                            $('#customerFormModal').modal('hide');
                        } else if (err.data.errorCode === 14) {
                        // Only images are allowed (Formats supported: png, jpg, jpeg).
                            this.message = 'admin.customers.create-customer.message.invalidFile';
                            this.isFailure = true;
                            this.notify(this.message, 'danger', 5000);
                        } else if (err.data.errorCode === 32) {
                            // "errorCode":32,"data":"Commercial Register is duplicate"
                            this.message = 'admin.customers.create-customer.message.commercialRegisterIsDuplicate';
                            this.isFailure = true;
                            this.notify(this.message, 'danger', 5000);
                            $('#customerFormModal').modal('hide');
                        } else if (err.data.errorCode === 31) {
                            // "errorCode":31,"data":"Email or MobilePhone is duplicate"
                            this.message = 'admin.customers.create-customer.message.emailOrMobilePhoneIsDuplicate';
                            this.isFailure = true;
                            this.notify(this.message, 'danger', 5000);
                            $('#customerFormModal').modal('hide');
                        } else {
                            this.message = 'admin.customers.create-customer.message.failed_creation';
                            this.isFailure = true;
                            this.notify(this.message, 'danger', 5000);
                            $('#customerFormModal').modal('hide');
                        }
    ...
    uploadFile(file, errFiles, imgFor) {
        ...
        if (extFile !== 'jpg' && extFile !== 'jpeg' && extFile !== 'png' && extFile !== 'gif' && extFile !== 'pdf') {
            this.notify('auth.register.customer.message.invalidFile', 'warning', 7000);
            switch (imgFor) {
                case 'coverPhoto' : {
            ...
            this.notify('auth.register.customer.message.invalidFileSize', 'warning', 7000);
            switch (imgFor) {
                case 'coverPhoto' : {
                    this.formData.coverPhoto = null;
                    break;
...
CustomerFormCtrl.$inject = ['CustomerService', '$translate', '$rootScope', 'Upload',
    'AppConstants'];
...
const CustomerFormComponent = {
    ...
    templateUrl: 'app/admin/supplier/customer-form/customer-form.component.html',
